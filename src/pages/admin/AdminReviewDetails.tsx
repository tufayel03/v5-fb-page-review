...
import { Star, Calendar, Trash2, Link as LinkIcon, User, Save, RefreshCw } from "lucide-react";
...
export default function AdminReviewDetails() {
  ...
  const handleSave = () => {
    ...
    fetch(`/api/admin/reviews/${id}`, {
      ...
      .then(data => {
        ...
        else alert(t("Review updated successfully"));
      });
  };
  ...
  const handleDeleteImage = async (indexToDelete: number) => {
    if (!window.confirm(t("Delete this proof image? This will permanently remove the file."))) return;
    ...
    try {
      ...
      alert(t('Failed to delete image'));
    }
    setDeletingImage(null);
  };

  const handleDeleteAllImages = async () => {
    if (!window.confirm(t("Delete ALL proof images for this review? This cannot be undone."))) return;
    setDeletingImage(-1);
    try {
      const res = await fetch(`/api/admin/reviews/${id}/proof-image`, {
        method: 'DELETE',
      ...
      alert(t('Failed to delete images'));
    }
    setDeletingImage(null);
  };
  ...
  const handleDelete = () => {
    fetch(`/api/admin/reviews/${id}`, {
      ...
      .catch(err => {
         console.error(err);
         alert(t("Failed to delete review: ") + err.message);
      });
  };

  if (loading) return <div className="p-10 text-center font-bold text-slate-400 animate-pulse">{t("Loading...")}</div>;
  if (!review) return <div className="p-10 text-center text-rose-500 font-bold">{t("Review not found")}</div>;

  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight">{t("Review Details")}</h1>
          <p className="text-slate-400 text-sm font-semibold mt-1">{t("Manage review information and status")}</p>
        </div>
        <div className="flex items-center gap-3">
          {deleteConfirm ? (
            ...
            <button onClick={() => setDeleteConfirm(true)} className="bg-white/5 border border-white/5 text-rose-400 px-4 py-2 flex items-center gap-2 rounded-lg text-sm font-bold hover:bg-white/10 transition-all">
              <Trash2 className="h-4 w-4" /> {t("Delete Review")}
            </button>
          )}
          <button onClick={handleSave} disabled={saving} className="bg-emerald-600 text-white px-4 py-2 flex items-center gap-2 rounded-lg text-sm font-bold hover:bg-emerald-700 transition-colors disabled:opacity-50 shadow-lg shadow-emerald-500/15">
             {saving ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} {t("Save Changes")}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
           <div className="bg-[#091124] rounded-xl border border-white/5 p-6 shadow-xl">
              <h2 className="font-bold border-b border-white/5 pb-2 mb-4 text-white">{t("Review Content")}</h2>
              <div className="mb-4 text-emerald-400 flex items-center space-x-1">
                 ...
                 <span className="font-bold ml-2 text-slate-300">{n(review.star_rating)}.0</span>
              </div>
              ...
              <div className="bg-[#050b18]/40 border border-white/5 rounded-lg p-4 grid grid-cols-2 gap-4 text-sm mb-4">
                 <div>
                    <div className="text-slate-400 font-bold mb-1 col-span-2">{t("Date of Experience")}</div>
                 ...
                 {review.order_amount && (
                 <div>
                    <div className="text-slate-400 font-bold mb-1 text-xs uppercase tracking-wider">{t("Order Amount")}</div>
                    <div className="font-bold text-emerald-400 mt-1">৳ {n(review.order_amount)}</div>
                 </div>
                 )}
              </div>
              ...
              {review.proof_image && (
                 <div className="mt-4">
                    <div className="flex items-center justify-between mb-3">
                       <div className="text-slate-400 font-bold text-sm uppercase tracking-wider">{t("Proof Image(s)")}</div>
                       <button
                         type="button"
                       ...
                       >
                         <Trash2 className="h-3.5 w-3.5" /> {t("Delete All Images")}
                       </button>
                    </div>
                    <div className="flex flex-wrap gap-4">
                        {(() => {
                           ...
                           if (images.length === 0) return <span className="text-slate-500 font-semibold text-sm">{t("No proof images.")}</span>;
                             ...
                             <div key={idx} className="relative group max-w-[200px]">
                               ...
                               {deletingImage === idx ? (
                                 ...
                                 <button 
                                   ...
                                   title={t("Delete this image (removes file from server)")}
                                 >
           ...
           <div className="bg-[#091124] rounded-xl border border-white/5 p-6 shadow-xl">
              <h2 className="font-bold border-b border-white/5 pb-2 mb-4 text-white">{t("Evidences & Links")}</h2>
              <div className="space-y-4 text-sm">
                 {review.bkash_number && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border border-rose-500/10 p-4 rounded-lg bg-rose-500/5">
                       <div><span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider mb-1">{t("bKash Number")}</span><span className="font-black text-slate-250">{n(review.bkash_number)}</span></div>
                       <div><span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider mb-1">{t("Account Type")}</span><span className="font-medium text-slate-350">{t(review.bkash_account_type || 'Unknown')}</span></div>
                       <div><span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider mb-1">{t("Display Name")}</span><span className="font-medium text-slate-350">{review.bkash_display_name || 'N/A'}</span></div>
                    </div>
                 )}
                 {review.facebook_post_link && (
                    <a href={review.facebook_post_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-emerald-450 hover:text-emerald-300 font-bold transition-all">
                      <LinkIcon className="h-4 w-4" /> {t("View Associated Facebook Post")}
                    </a>
                 )}
              </div>
           </div>
        ...
        <div className="space-y-6">
           <div className="bg-[#091124] rounded-xl border border-white/5 p-6 shadow-xl">
              <h2 className="font-bold border-b border-white/5 pb-2 mb-4 text-white">{t("Moderation")}</h2>
              <div className="space-y-4 text-sm">
                 <div>
                    <label className="block text-slate-500 font-bold mb-1">{t("Status")}</label>
                 ...
                 <div>
                      <label className="block text-slate-500 font-bold mb-1">{t("Review Type")}</label>
                 ...
                 <div className="flex items-center justify-between border-t border-white/5 pt-4 mt-2">
                    <span className="text-slate-400 font-bold text-sm">{t("Share Image Publicly")}</span>
                    <button
                       type="button"
           ...
           <div className="bg-[#091124] rounded-xl border border-white/5 p-6 shadow-xl">
              <h2 className="font-bold border-b border-white/5 pb-2 mb-4 text-white">{t("Metadata")}</h2>
              <div className="space-y-3 text-sm">
                 <div>
                    <span className="text-slate-400 font-bold text-xs uppercase tracking-wider mb-2 block">{t("Review ID")}</span>
                 ...
                 <div>
                      <span className="text-slate-400 font-bold text-xs uppercase tracking-wider mb-2 block">{t("Author")}</span>
                      <div className="flex items-center gap-2 mt-1">
                         <div className="bg-white/5 border border-white/5 p-1 rounded-full"><User className="h-3 w-3 text-slate-400" /></div>
                         <span className="font-bold text-slate-200">{review.author_name || review.author_username || t('Unknown User')}</span>
                      </div>
                 </div>
                 <div>
                    <span className="text-slate-400 font-bold text-xs uppercase tracking-wider mb-2 block">{t("Target Page")}</span>
                    <div className="flex items-center gap-2 mt-1">
                        <span className="font-bold text-emerald-400 truncate block max-w-[200px]">{review.current_name || t('Unknown Page')}</span>
                 ...
                 <div>
                    <span className="text-slate-400 font-bold text-xs uppercase tracking-wider mb-2 block">{t("Created At")}</span>

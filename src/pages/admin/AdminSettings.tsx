...
export default function AdminSettings() {
  ...
  const checkCookieStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCookieCheckResult({ status: 'error', message: t('Failed to communicate with server status check.') });
      }
    } catch (err: any) {
      setCookieCheckResult({ status: 'error', message: t('Network error verifying cookie status.') });
    } finally {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setMessage({ type: 'success', text: t('Logo uploaded successfully. Remember to Save Changes!') });
        } else {
          setMessage({ type: 'error', text: t('Failed to upload logo image') });
        }
      } catch (err) {
        console.error(err);
        setMessage({ type: 'error', text: t('An error occurred during logo upload') });
      } finally {
  ...
  const allTabs = [
    ...
    { id: "branding", label: t("Website Branding") },
    { id: "reviews", label: t("Review Settings") },
    { id: "claims", label: t("Claim Settings") },
    { id: "contact-numbers", label: t("bKash / Numbers") },
    { id: "advertising", label: t("Advertising Settings") },
    { id: "blog-seo", label: t("Blog & SEO") },
    { id: "notifications", label: t("Notifications") },
    { id: "smtp", label: t("Email / SMTP Config") },
    { id: "security", label: t("Security Settings") },
    { id: "roles-permissions", label: t("Roles & Permissions") },
    { id: "import-export", label: t("Import / Export") },
    { id: "legal-policy", label: t("Legal & Policy") },
  ...
  const handleSave = () => {
    ...
    if (emailKey && !emailRegex.test(emailKey)) {
      setMessage({ type: 'error', text: t('Invalid Contact Email format') });
      return;
    }
    if (supportEmailKey && !emailRegex.test(supportEmailKey)) {
      setMessage({ type: 'error', text: t('Invalid Support Email format') });
      return;
    }
  ...
  if (loading) return <div className="p-10 text-center font-bold text-slate-400">{t("Loading settings...")}</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">{t("Manage website, review, claim, security, SEO, and admin preferences.")}</p>
        </div>
        <div className="flex items-center gap-3">
          <button 
            ...
            onClick={() => { if(window.confirm(t('Reset unsaved changes?'))) fetchSettings(); }}
          >
            {t("Reset")}
          </button>
          <button 
      ...
      {message && (
        <div className={`p-4 rounded-lg flex items-center gap-3 ${message.type === 'error' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/15' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/15'}`}>
          ...
          <p className="font-bold text-sm">{t(message.text)}</p>
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="flex-1 w-full bg-[#091124] border border-white/5 rounded-xl shadow-xl p-6 lg:p-8 min-h-[600px]">
          ...
          {activeTab === "general" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("General Settings")}</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Site Name")}</label>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Site Tagline")}</label>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Contact Email")}</label>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Facebook Page URL")}</label>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Support Email")}</label>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Country")}</label>
                  <select value={getSettingValue('country', 'BD')} onChange={e => updateSetting('country', e.target.value, 'general')} className="w-full border border-white/5 bg-[#050b18]/45 text-white rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500/20 outline-none">
                    <option value="BD" className="bg-[#091124]">{t("Bangladesh")}</option>
                    <option value="IN" className="bg-[#091124]">{t("India")}</option>
                    <option value="US" className="bg-[#091124]">{t("United States")}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Currency")}</label>
                  <select value={getSettingValue('currency', 'BDT')} onChange={e => updateSetting('currency', e.target.value, 'general')} className="w-full border border-white/5 bg-[#050b18]/45 text-white rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500/20 outline-none">
                    <option value="BDT" className="bg-[#091124]">{t("BDT (৳)")}</option>
                    <option value="USD" className="bg-[#091124]">{t("USD ($)")}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Timezone")}</label>
                  <select value={getSettingValue('timezone', 'Asia/Dhaka')} onChange={e => updateSetting('timezone', e.target.value, 'general')} className="w-full border border-white/5 bg-[#050b18]/45 text-white rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500/20 outline-none">
                    <option value="Asia/Dhaka" className="bg-[#091124]">{t("Asia/Dhaka (GMT+6)")}</option>
                    <option value="UTC" className="bg-[#091124]">{t("UTC")}</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Language")}</label>
                  <select value={getSettingValue('language', 'en')} onChange={e => updateSetting('language', e.target.value, 'general')} className="w-full border border-white/5 bg-[#050b18]/45 text-white rounded-lg p-2.5 focus:ring-2 focus:ring-emerald-500/20 outline-none">
                    <option value="en" className="bg-[#091124]">{t("English")}</option>
                    <option value="bn" className="bg-[#091124]">{t("Bengali")}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="mt-6 border-t border-white/5 pt-6 space-y-4">
                <div>
                  <label className="block text-sm font-bold text-slate-300">{t("Facebook Cookie String / JSON (For Scraping)")}</label>
                  <p className="text-xs text-slate-400 mt-1">{t("Paste your active Facebook session cookie string or raw exported JSON array from Cookie-Editor here. This is used by the server to safely crawl, authenticate, and search profiles directly when URLs are pasted in the search box.")}</p>
                </div>
                ...
                <div className="p-4 border border-white/5 bg-[#050b18]/30 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-black text-slate-400 uppercase tracking-wider">{t("Cookie Session Status:")}</span>
                      {cookieChecking ? (
                        <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                          <RefreshCw className="h-3 w-3 animate-spin" /> {t("Verifying Live...")}
                        </span>
                    ...
                    <p className="text-xs text-slate-400 font-semibold leading-relaxed">
                      {cookieChecking 
                        ? t('Checking live authentication link to Facebook servers...') 
                        : cookieCheckResult?.message ? t(cookieCheckResult.message) : t('Click live verify to test current session credentials.')}
                    </p>
                  </div>
                  ...
                  >
                    ...
                    {t("Live Verify Session")}
                  </button>
                </div>
              </div>

              <div className="mt-6 border-t border-white/5 pt-6 space-y-2">
                <label className="block text-sm font-bold text-slate-300">{t("Exclude IPs from Visitor Count")}</label>
                <p className="text-xs text-slate-400">{t("Enter comma-separated IP addresses (e.g. 103.92.155.92, 192.168.1.1) to exclude them from the visitor and traffic analytics on the dashboard overview.")}</p>
          ...
          {activeTab === "branding" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Website Branding")}</h2>
              
              <div className="grid grid-cols-1 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Primary Color (Hex)")}</label>
                  <div className="flex items-center gap-3">
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-300 mb-1">{t("Site Logo")}</label>
                  <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                    <div className="flex-1 w-full space-y-2">
                      <input 
                        ...
                        placeholder={t("Or enter logo URL: https://example.com/logo.png")} 
                      />
                      <div className="flex items-center gap-2">
                        <input
                          type="file"
                        ...
                        >
                          ...
                          <span>{logoUploading ? t('Uploading...') : t('Upload Local Logo')}</span>
                        </button>
                    ...
                    {getSettingValue('site_logo', '') ? (
                      ...
                      <div className="bg-[#050b18]/45 border border-white/5 border-dashed p-3 rounded-xl flex items-center justify-center shrink-0 min-w-[128px] max-w-[192px] h-20">
                        <span className="text-xs text-slate-500 font-medium">{t("No Logo Saved")}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Homepage Hero Title")}</label>
                ...
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Homepage Hero Subtitle")}</label>
                ...
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Footer Description")}</label>
          ...
          {activeTab === "reviews" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Review Settings")}</h2>
              ...
              <div className="space-y-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Auto approve positive/neutral reviews")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Require admin approval for Fraud Reports")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Limit one review per user per page")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Allow users to upload image proof in reviews")}</span>
                </label>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                 <div>
                    <label className="block text-sm font-bold text-slate-300 mb-1">{t("Max Description Length")}</label>
                 ...
                 <div>
                    <label className="block text-sm font-bold text-slate-300 mb-1">{t("Min Description Length")}</label>
          ...
          {activeTab === "advanced" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold flex items-center gap-2 text-rose-400 border-b border-white/5 pb-4 mt-8">
                <ShieldAlert className="h-5 w-5" /> {t("Danger Zone & Advanced")}
              </h2>
              ...
              <div className="space-y-4">
                <label className="flex items-center gap-3 cursor-pointer p-4 border border-rose-500/15 rounded-lg bg-rose-500/5">
                  ...
                  <div>
                     <span className="font-bold text-rose-400 block">{t("Maintenance Mode")}</span>
                     <span className="text-xs font-medium text-rose-300">{t("Disables public access. Only admins can view the site.")}</span>
                  </div>
                </label>

                <div className="flex items-center justify-between p-4 border border-white/5 bg-[#050b18]/45 rounded-lg">
                   <div>
                     <h3 className="font-bold text-slate-200">{t("Clear Application Cache")}</h3>
                     <p className="text-sm text-slate-400">{t("Clears compiled search indexes and fast-loading cached data.")}</p>
                   </div>
                   <button className="px-4 py-2 bg-white/5 border border-white/5 text-slate-300 font-bold rounded hover:bg-white/10 text-sm transition-colors">{t("Clear Cache")}</button>
                </div>
                
                <div className="flex items-center justify-between p-4 border border-white/5 bg-[#050b18]/45 rounded-lg">
                   <div>
                     <h3 className="font-bold text-slate-200">{t("Full Backup (DB + Uploads)")}</h3>
                     <p className="text-sm text-slate-400">{t("Download a complete archive of all database contents and user uploads.")}</p>
                   </div>
                   <a href="/api/admin/backup-db" download className="px-4 py-2 bg-blue-500/10 border border-blue-500/15 font-bold text-blue-400 rounded hover:bg-blue-500/20 text-sm flex items-center gap-2 transition-all">
                     <Download className="w-4 h-4" /> {t("Backup DB")}
                   </a>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-rose-500/15 rounded-lg bg-rose-500/5">
                   <div className="mb-3 sm:mb-0">
                     <h3 className="font-bold text-rose-400">{t("Restore Full Backup")}</h3>
                     <p className="text-sm text-rose-300 max-w-md">{t("Upload a full backup or a database file. This will {{action}} all current data, uploads and configuration. The server will restart automatically.", { action: t("OVERWRITE") })}</p>
                   </div>
                   <div>
                     <input type="file" id="db-upload" className="hidden" accept=".db,.sqlite,.sqlite3,.zip" onChange={async (e) => {
                       const file = e.target.files?.[0];
                       if (!file) return;
                       if (!window.confirm(t('WARNING: This will OVERWRITE ALL existing data & user uploads! This action cannot be undone. Are you absolutely sure you want to proceed?'))) return;
                       ...
                       try {
                         ...
                         if (res.ok) {
                           alert(t('Full Website Data restored successfully. The server is restarting, please log in again after a few seconds.'));
                           window.location.reload();
                         } else {
                           alert(t('Failed to restore database.'));
                         }
                       } catch (err) {
                         alert(t('Error occurred while restoring the database.'));
                       }
                     }} />
                     <button onClick={() => document.getElementById('db-upload')?.click()} className="px-4 py-2 bg-rose-500/10 border border-rose-500/15 font-bold text-rose-405 rounded hover:bg-rose-500/20 text-sm flex items-center gap-2 transition-all">
                       <Upload className="w-4 h-4" /> {t("Restore Backup")}
                     </button>
                   </div>
                </div>
              </div>
          ...
          {activeTab === "claims" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Claim Settings")}</h2>
              <div className="space-y-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Enable page claim system")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Manual approval only")}</span>
                </label>
              </div>
              <div className="grid grid-cols-1 gap-6 mt-6">
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Official FB Page URL (For claims verification)")}</label>
                ...
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Claim Instruction Text")}</label>
          ...
          {activeTab === "contact-numbers" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("bKash / Contact Number Settings")}</h2>
              <div className="space-y-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Allow bKash number submission")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Allow Nagad number submission")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Show numbers publicly")}</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-slate-300">{t("Mask numbers publicly (e.g., 01XXX-XXX-123)")}</span>
                </label>
              </div>
            </div>
          )}
          ...
          {activeTab === "blog-seo" && (
            <div className="space-y-6">
              <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Blog & SEO Settings")}</h2>
              <div className="grid grid-cols-1 gap-6">
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Default Meta Title")}</label>
                ...
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Default Meta Description")}</label>
                ...
                <div>
                   <label className="block text-sm font-bold text-slate-300 mb-1">{t("Page Profile SEO Title Format")}</label>
          ...
          {activeTab === "notifications" && (
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Notification Settings")}</h2>
                 <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <h3 className="font-bold text-slate-200">{t("Admin Email Notifications")}</h3>
                      <label className="flex items-center gap-3 cursor-pointer">
                         ...
                         <span className="text-sm font-bold text-slate-300">{t("New review submitted")}</span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer">
                         ...
                         <span className="text-sm font-bold text-slate-300">{t("New fraud report submitted")}</span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer">
                         ...
                         <span className="text-sm font-bold text-slate-300">{t("New page claim request")}</span>
                      </label>
                    </div>
                    <div className="space-y-4">
                       <h3 className="font-bold text-slate-200">{t("System Emails")}</h3>
                       <div>
                          <label className="block text-sm font-bold text-slate-300 mb-1">{t("From Email Address")}</label>
                       ...
                       <div>
                          <label className="block text-sm font-bold text-slate-300 mb-1">{t("From Name")}</label>
              ...
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("SMTP Email Configuration (Namecheap)")}</h2>
                 <div className="grid md:grid-cols-2 gap-6 max-w-2xl">
                    <div className="md:col-span-2">
                         <p className="text-sm text-slate-400">{t("Configure your Namecheap Private Email or other SMTP settings to enable forgot password features and system emails.")}</p>
                    </div>
                    <div className="md:col-span-2">
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("SMTP Host")}</label>
                    ...
                    <div>
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("SMTP Port")}</label>
                    ...
                    <div>
                       <label className="flex items-center gap-3 cursor-pointer pt-8">
                         ...
                         <span className="text-sm font-bold text-slate-300">{t("Use SSL/TLS (Secure)")}</span>
                       </label>
                    </div>
                    <div className="md:col-span-2">
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("SMTP Username (Email Address)")}</label>
                    ...
                    <div className="md:col-span-2">
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("SMTP Password")}</label>
                    ...
                    <div className="md:col-span-2">
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("From Name & Email Fallback")}</label>
                       <p className="text-xs text-slate-400 mb-2">{t("Configure these under the Notifications tab over System Emails.")}</p>
                    </div>
                 </div>
              </div>
          )}
          ...
          {activeTab === "security" && (
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Security Settings")}</h2>
                 <div className="space-y-4 max-w-sm">
                    <div>
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("Login Attempt Limit")}</label>
                    ...
                    <div>
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("Lockout Duration (Minutes)")}</label>
                    ...
                    <div>
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("Minimum Password Length")}</label>
                    ...
                    <label className="flex items-center gap-3 cursor-pointer pt-2">
                      ...
                      <span className="text-sm font-bold text-slate-300">{t("Require strong password (Uppercase, Number, Special Char)")}</span>
                    </label>
                 </div>
              </div>
          )}
          ...
          {activeTab === "roles-permissions" && (
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Roles & Permissions")}</h2>
                 <div className="bg-[#050b18]/45 border border-white/5 rounded-lg p-6">
                    <p className="font-bold text-slate-200 mb-2">{t("Role Management Details")}</p>
                    <ul className="list-disc list-inside text-sm text-slate-350 space-y-1">
                       <li><b>{t("Super Admin:")}</b> {t("Full access to all settings and deletion capabilities.")}</li>
                       <li><b>{t("Admin:")}</b> {t("Cannot modify security settings or delete Super Admins.")}</li>
                       <li><b>{t("Moderator:")}</b> {t("Can approve reviews and claims, but no settings access.")}</li>
                    </ul>
                 </div>
              </div>
          )}
          ...
          {activeTab === "import-export" && (
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Import / Export Settings")}</h2>
                 <div className="space-y-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <span className="text-sm font-bold text-slate-300">{t("Enable bulk import from dashboard")}</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <span className="text-sm font-bold text-slate-300">{t("Enable bulk export")}</span>
                    </label>
                    <div className="max-w-sm pt-2">
                       <label className="block text-sm font-bold text-slate-300 mb-1">{t("Export Row Limit")}</label>
              ...
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Legal & Policy Settings")}</h2>
                 <div className="grid grid-cols-1 gap-6">
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Global Review Disclaimer")}</label>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Privacy Policy Link URL")}</label>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Terms of Service Link URL")}</label>
          ...
          {activeTab === "advertising" && (
              <div className="space-y-6">
                 <h2 className="text-xl font-bold text-white border-b border-white/5 pb-4">{t("Advertising & Verification Settings")}</h2>
                 <p className="text-sm text-slate-400">
                   {t("Manage scripts, ads, and verification tags from Google AdSense, Adsterra, or other networks.")}
                 </p>
                 <div className="grid grid-cols-1 gap-6">
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Verify Site Ownership / Head Code (Google AdSense Code snippet, Ads.txt snippet, Meta tag)")}</label>
                      ...
                      <p className="text-xs text-slate-500 mt-1">{t("To get your site ready to show ads, copy and paste your AdSense verification code snippet or site ownership meta tags here. It will automatically load on all pages.")}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Profile Page Left Sidebar Adsterra Banner Ad Code (160x600 Skyscraper)")}</label>
                      ...
                      <p className="text-xs text-slate-500 mt-1">{t("Paste the exact HTML or Javascript snippet (typically 160x600 size) provided by your Adsterra publisher dashboard.")}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Homepage Adsterra Banner Ad Code (HTML / Script / iframe)")}</label>
                      ...
                      <p className="text-xs text-slate-500 mt-1">{t("Paste the exact HTML or Javascript snippet provided by your Adsterra publisher dashboard.")}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-slate-300 mb-1">{t("Google AdSense Banner Ad Code (HTML / Script)")}</label>

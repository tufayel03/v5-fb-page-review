...
export default function AdminLogs() {
  ...
  if (loading) return <div className="p-10 text-center font-bold text-slate-500">Loading logs...</div>;

  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight">Admin Logs</h1>
          <p className="text-slate-400 text-sm font-semibold mt-1">System activity and audit logs for admin actions.</p>
        </div>
        <button
      ...
      <div className="bg-[#091124] border border-white/5 rounded-xl shadow-xl overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#050b18]/60 text-slate-400 uppercase font-bold text-xs">
              <tr>
                ...
                <th className="px-6 py-4 border-b border-white/5 cursor-pointer hover:bg-white/5" onClick={() => handleSort("admin_name")}>
                  <div className="flex items-center gap-1">Admin <ArrowUpDown className="h-3 w-3"/></div>
                </th>
            ...
            <tbody className="divide-y divide-white/5">
               {filteredAndSortedLogs.length === 0 ? (
                 <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-slate-500 font-bold italic">No logs found</td>
                 </tr>
               ) : (
                   ...
                   <tr key={log.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                         <div className="flex items-center gap-3">
                            ...
                            <div>
                               <div className="text-sm font-bold text-slate-200">{log.admin_name || 'System Admin'}</div>
                            </div>
                         </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="p-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between bg-[#050b18]/40 gap-4">
           <div className="text-sm text-slate-400 font-medium">
              Showing {filteredAndSortedLogs.length === 0 ? 0 : startIndex + 1} to {Math.min(startIndex + itemsPerPage, filteredAndSortedLogs.length)} of {filteredAndSortedLogs.length} entries
           </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#050b18]/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-[#091124] border border-white/10 rounded-xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-xl font-bold text-white mb-2">Delete All Logs?</h3>
            <p className="text-slate-400 font-semibold text-sm mb-6">Are you sure you want to permanently delete ALL admin logs? This action cannot be undone.</p>

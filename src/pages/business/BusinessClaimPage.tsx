...
export default function BusinessClaimPage({ onSuccess }: BusinessClaimPageProps) {
    ...
    try {
      ...
      if (verifiedPage && verifiedPage.id) {
        if (verifiedPage.claim_status === 'Claimed') {
          setPage(null);
          setTargetPageId(null);
          setSearchError('This page has already been claimed.');
          return null;
        } else {
      ...
      setSearchError('Failed to verify page URL.');
      return null;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentTargetId) {
      if (!pageUrl) {
         setError('No page selected to claim. Please enter a valid Facebook Page URL.');
         setLoading(false);
         return;
      }
    ...
    if (isNew && !pageName.trim()) {
      setError('This Facebook page is not listed yet. Please enter a Page Name in the field below to list and claim it.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(errData.error || 'Failed to submit claim request.');
      }
    } catch(err) {
    ...
    return (
      <div id="claim-success-screen" className="bg-white p-8 border border-neutral-200 rounded-3xl shadow-sm text-center py-12 space-y-6 max-w-xl mx-auto">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-slate-900">Claim Request Submitted!</h2>
          <p className="text-slate-500 font-medium leading-relaxed">
            Your claim request for <span className="font-extrabold text-emerald-800">{pageName || pageUrl}</span> has been received, pending verification.
          </p>
        </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 text-left space-y-2 dark:bg-amber-500/5">
          <p className="text-xs font-black text-amber-800 dark:text-amber-300 uppercase tracking-wider flex items-center gap-1.5 leading-none">
            <AlertCircle className="h-4 w-4 shrink-0 text-amber-500" /> Ownership Verification Needed
          </p>
          <p className="text-xs text-amber-700 dark:text-amber-200 font-semibold leading-relaxed">
            To activate ownership, you{" "}
            {facebookPageUrl ? (
              <a
              ...
              >
                <Facebook className="h-3.5 w-3.5 fill-[#1877f2] dark:fill-[#5890ff] shrink-0" /> must message us
              </a>
            ) : (
              <span className="underline font-black">must message us</span>
            )}{" "}
            your claim username (<strong className="text-indigo-900 dark:text-indigo-300 font-bold">{user?.username}</strong>) or email (<strong className="text-[#1877f2] dark:text-[#5890ff] font-bold">{user?.email}</strong>){" "}
            <strong className="font-black text-amber-900 dark:text-amber-100">directly from the claimed Facebook Page's inbox</strong> to our official page.
          </p>
          {facebookPageUrl && (
            <a
              href={facebookPageUrl}
            ...
            >
              Go to Our Facebook Page <ExternalLink className="h-2.5 w-2.5" />
            </a>
          )}
        </div>

        <p className="text-xs text-slate-400 font-medium">
          Redirecting automatically to your business dashboard pages...
        </p>
        <button 
          id="btn-goto-pages"
          onClick={() => {
        ...
        >
          Go to My Pages <ArrowRight className="h-4 w-4" />
        </button>
      </div>
    );
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-2 text-slate-900">Claim Your Facebook Page</h1>
      <p className="text-slate-500 mb-6">Verify ownership of your Facebook page to reply to reviews and build trust.</p>
      ...
      <div className="mb-6 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl flex items-start gap-3 dark:bg-indigo-950/20">
        ...
        <div className="space-y-1">
          <p className="text-sm font-extrabold text-indigo-900 dark:text-indigo-300">How claiming works:</p>
          <p className="text-xs text-indigo-800 dark:text-indigo-200 font-semibold leading-relaxed">
            After submitting your claim request, you must{" "}
            {facebookPageUrl ? (
              <a
                href={facebookPageUrl}
                target="_blank"
                ...
                className="inline-flex items-center gap-1 text-[#1877f2] dark:text-[#5890ff] font-extrabold underline hover:no-underline align-middle"
              >
                <Facebook className="h-3.5 w-3.5 fill-[#1877f2] dark:fill-[#5890ff] shrink-0" /> send us a message
              </a>
            ) : (
              <span className="underline font-black">send us a message</span>
            )}{" "}
            with your claim username/email to our official Facebook page <strong className="font-extrabold text-[#1877f2] dark:text-[#5890ff]">directly from your Facebook Page you are claiming</strong> to complete the verification.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-8 border border-neutral-200 rounded-3xl shadow-sm">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          {!pageId && (
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-1.5">Facebook Page URL</label>
              <input
                id="input-page-url"
                type="url"
                required
          ...
          {isNewPage && (
            <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 space-y-3">
              <div className="flex items-start gap-2.5">
                ...
                <div>
                   <h3 className="font-bold text-slate-900 text-sm">Create New Page Listing</h3>
                   <p className="text-xs text-slate-500">This Facebook page isn't listed on our portal yet. Give it a name to create its profile and submit your claim request automatically!</p>
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1.5">Page Name</label>
                <input 
                  id="input-page-name"
                  type="text" 
                  required
          ...
          {page && !isNewPage && (
            <div className="p-4 bg-emerald-50/80 rounded-2xl border border-emerald-100/80 flex items-center gap-3">
               ...
               <p className="text-sm font-medium text-emerald-800">
                 Claiming Listing: <span className="font-extrabold">{page.current_name}</span>
               </p>
            </div>
          )}
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1.5">Contact Phone (Optional)</label>
            <input 
              ...
              placeholder="Optionally provide a contact number" 
            />
          </div>
          <button 
            id="btn-submit-claim"
          ...
          >
            {loading ? 'Processing...' : 'Submit Claim Request'}
          </button>
        </form>
      </div>
    </div>

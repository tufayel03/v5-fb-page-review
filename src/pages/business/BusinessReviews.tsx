...
export default function BusinessReviews() {
  ...
  const handleReplySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(t('Your reply has been posted.'));
        setReplyModalOpen(false);
        fetchReviews();
      } else {
        const err = await res.json();
        alert(err.error || t('Failed to submit reply'));
      }
    } catch (err) {
  ...
  const handleDisputeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(t('Your dispute has been submitted.'));
        setDisputeModalOpen(false);
        fetchReviews();
      } else {
        const err = await res.json();
        alert(err.error || t('Failed to submit dispute'));
      }
    } catch (err) {
      alert(t('An error occurred'));
    }
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 font-medium tracking-wide">{t("Loading reviews...")}</div>;
  ...
  return (
    <div className={`space-y-6 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className={`text-2xl font-bold tracking-tight ${cText}`}>{t("Reviews")}</h1>
          <p className={cTextMuted}>{t("View reviews for your claimed pages, reply publicly, or submit disputes.")}</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-7 gap-3">
        {[
          { label: t('Total'), value: totalReviews, icon: Database, color: '#3b82f6', border: 'border-t-blue-500/50' },
          { label: t('Avg Rating'), value: averageRating, icon: Star, color: '#eab308', border: 'border-t-yellow-500/50' },
          { label: t('Good'), value: goodReviews, icon: ThumbsUp, color: '#10b981', border: 'border-t-emerald-500/50' },
          { label: t('Bad'), value: badReviews, icon: ThumbsDown, color: '#f97316', border: 'border-t-orange-500/50' },
          { label: t('Fraud'), value: fraudReports, icon: ShieldAlert, color: '#ef4444', border: 'border-t-rose-500/50' },
          { label: t('Pending Replies'), value: pendingReplies, icon: Clock, color: '#8b5cf6', border: 'border-t-purple-500/50' },
          { label: t('Open Disputes'), value: openDisputes, icon: Scale, color: '#a855f7', border: 'border-t-fuchsia-500/50' },
        ].map(stat => (
          <div key={stat.label} className={`p-4 rounded-xl border border-b-0 border-x-0 bg-opacity-70 ${theme === 'light' ? `bg-white shadow-sm ${stat.border}` : `bg-[#131A2B] border-white/5 ${stat.border}`}`} style={{ borderTopWidth: '2px' }}>
             <div className="flex items-center gap-3">
               ...
               <div>
                  ...
                  <p className={`text-xl font-bold mt-1 ${theme === 'light' ? 'text-slate-900' : ''}`} style={{ color: theme === 'light' ? undefined : stat.color }}>{stat.label === t('Avg Rating') ? n(stat.value) : n(Number(stat.value))}</p>
               </div>
             </div>
          </div>
        ))}
      ...
      <div className={`p-5 rounded-2xl border shadow-sm ${cContainer}`}>
        <div className="flex flex-col xl:flex-row gap-4 mb-4">
           <div className="flex-1 relative">
              ...
              <input 
                 type="text" 
                 placeholder={t("Search reviews (title, description, username, bkash)...")}
                 value={searchQuery}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className={`rounded-xl px-4 py-2.5 text-sm appearance-none outline-none ${cInput}`}>
            ...
            <option value="Fraud Report">{t("Fraud Report")}</option>
          </select>
          <select value={ratingFilter} onChange={e => setRatingFilter(e.target.value)} className={`rounded-xl px-4 py-2.5 text-sm appearance-none outline-none ${cInput}`}>
            ...
            {[5,4,3,2,1].map(num => <option key={num} value={num.toString()}>{n(num)} {t("Stars")}</option>)}
          </select>
          <select value={replyFilter} onChange={e => setReplyFilter(e.target.value)} className={`rounded-xl px-4 py-2.5 text-sm appearance-none outline-none ${cInput}`}>
            <option value="">{t("All Reply Status")}</option>
            <option value="Replied">{t("Replied")}</option>
            <option value="Not Replied">{t("Not Replied")}</option>
          </select>
          <select value={disputeFilter} onChange={e => setDisputeFilter(e.target.value)} className={`rounded-xl px-4 py-2.5 text-sm appearance-none outline-none ${cInput}`}>
            <option value="">{t("All Dispute Status")}</option>
            <option value="Disputed">{t("Disputed")}</option>
            <option value="Not Disputed">{t("Not Disputed")}</option>
            <option value="Open Dispute">{t("Open Dispute")}</option>
            <option value="Resolved Dispute">{t("Resolved Dispute")}</option>
          </select>
          <select value={sortOrder} onChange={e => setSortOrder(e.target.value)} className={`rounded-xl px-4 py-2.5 text-sm appearance-none outline-none ${cInput}`}>
            ...
            <option value="Highest Rating">{t("Highest Rating")}</option>
            <option value="Lowest Rating">{t("Lowest Rating")}</option>
            <option value="Fraud Reports First">{t("Fraud Reports First")}</option>
            <option value="Suspicious Reports First">{t("Suspicious Reports First")}</option>
            <option value="Not Replied First">{t("Not Replied First")}</option>
          ...
          paginatedReviews.map((review, idx) => {
            ...
            <div key={review.id} className={`rounded-2xl border shadow-sm p-6 relative flex flex-col gap-5 ${cContainer}`}>
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                   ...
                   <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border ${
                      ...
                      {isFraud ? t('Fraud Report') : t(review.review_type)}
                   </span>
                   {review.dispute && (
                     <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border inline-flex items-center gap-1 ${
                        ...
                        <AlertCircle className="h-3 w-3" /> {t(review.dispute.status || "") || review.dispute.status} {t("Dispute")}
                     </span>
                   )}
                </div>
              ...
              <div className={`flex flex-wrap items-center text-sm gap-x-6 gap-y-3 ${cTextMuted}`}>
                <div className="flex items-center gap-2"><User className="h-4 w-4" /> <span className="font-semibold text-current opacity-70">{t("Reviewer:")}</span> {review.user_name || t('Anonymous')}</div>
                <div className="flex items-center gap-2"><Calendar className="h-4 w-4" /> <span className="font-semibold text-current opacity-70">{t("Posted:")}</span> {new Date(review.created_at).toLocaleDateString(dateLocale)}</div>
                {review.date_of_experience && <div className="flex items-center gap-2"><Calendar className="h-4 w-4" /> <span className="font-semibold text-current opacity-70">{t("Experience Date:")}</span> {review.date_of_experience}</div>}
                {review.bkash_number && <div className="flex items-center gap-2"><Phone className="h-4 w-4" /> <span className="font-semibold text-current opacity-70">{t("Contact/bKash:")}</span> {n(review.bkash_number)}</div>}
              </div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm font-semibold mt-1">
                  {review.proof_image && (
                    <button onClick={() => openProofModal(review.proof_image)} className="flex items-center gap-1.5 text-purple-500 hover:text-purple-400 transition-colors">
                      <Paperclip className="h-4 w-4" /> {t("Proof Attached")}
                    </button>
                  )}
                  {review.facebook_post_link && (
                    <a href={review.facebook_post_link} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-blue-500 hover:text-blue-400 transition-colors">
                      <Facebook className="h-4 w-4" /> {t("Facebook Evidence")}
                    </a>
                  )}
                </div>
              )}
              ...
              {review.owner_reply && (
                <div className={`p-4 rounded-xl border mt-2 ${theme === 'light' ? 'bg-indigo-50 border-indigo-100 text-slate-800' : 'bg-indigo-500/10 border-indigo-500/20 text-indigo-100'}`}>
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${theme === 'light' ? 'bg-indigo-200 text-indigo-800' : 'bg-indigo-50 text-indigo-900'}`}>{t("Business Owner Reply")}</span>
                    <span className="text-xs opacity-60">{new Date(review.owner_reply.created_at).toLocaleDateString(dateLocale)}</span>
                  </div>
              ...
              <div className={`flex flex-wrap items-center justify-between gap-3 pt-3 border-t ${theme === 'light' ? 'border-slate-100' : 'border-white/5'}`}>
                 <div className="flex items-center gap-3">
                   ...
                   >
                     <MessageSquare className="h-4 w-4" /> {review.owner_reply ? t('Edit Reply') : t('Reply')}
                   </button>
                   <button 
                 ...
                 <Link to={`/page/${review.page_id}`} target="_blank" className="text-sm font-bold text-indigo-500 hover:text-indigo-400 flex items-center gap-2 pr-2">
                   {t("View Public Review")} <ExternalLink className="h-4 w-4" />
                 </Link>
              </div>
            </div>
            );
      ...
      {totalPages > 0 && (
        <div className="flex justify-center items-center gap-4 mt-8 pb-4">
          <button disabled={currentPage === 1} onClick={() => setCurrentPage(c => c - 1)} className={`px-5 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50 transition-colors ${cInput}`}>{t("Previous")}</button>
          <span className={`text-sm font-bold ${cText}`}>{n(currentPage)} / {n(totalPages)}</span>
          <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(c => c + 1)} className={`px-5 py-2.5 rounded-xl text-sm font-bold disabled:opacity-50 transition-colors ${cInput}`}>{t("Next")}</button>
        </div>
      )}
        ...
        >
           <div className="relative max-w-4xl max-h-[90vh] w-full flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
              ...
              <img 
                ...
                alt={`${t("Proof Evidence")} ${proofIndex + 1}`} 
              ...
              <div className="mt-4 text-white/70 font-semibold text-sm bg-white/10 px-3 py-1 rounded-full font-sans">
                {n(proofIndex + 1)} / {n(proofImages.length)}
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className={`rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] border ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-[#0B1120] border-white/10'}`}>
            <div className={`p-5 border-b flex justify-between items-center ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-[#121A2F] border-white/5'}`}>
              <h2 className={`font-bold text-lg ${cText}`}>{t("Public Reply")}</h2>
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              ...
              <form id="reply-form" onSubmit={handleReplySubmit}>
                <label className={`block text-sm font-bold mb-2 ${theme === 'light' ? 'text-slate-700' : 'text-slate-300'}`}>{t("Your Response")}</label>
                <textarea 
                  rows={5}
                  value={replyText}
                  onChange={e => setReplyText(e.target.value)}
                  placeholder={t("Write your official response here. This will be visible publicly on the review page.")}
            ...
            <div className={`p-5 border-t flex justify-end gap-3 ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-[#121A2F] border-white/5'}`}>
              <button disabled={submitting} type="button" onClick={() => setReplyModalOpen(false)} className={`px-5 py-2.5 rounded-lg font-bold transition-colors ${theme === 'light' ? 'text-slate-600 hover:bg-slate-200' : 'text-slate-400 hover:bg-white/5'}`}>{t("Cancel")}</button>
              <button disabled={submitting} type="submit" form="reply-form" className="px-5 py-2.5 rounded-lg font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50">
                {submitting ? t('Submitting...') : t('Post Reply')}
              </button>
            </div>
          </div>
        </div>
      ...
      {disputeModalOpen && selectedReview && (
        <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className={`rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] border ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-[#0B1120] border-white/10'}`}>
            <div className={`p-5 border-b flex justify-between items-center ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-[#121A2F] border-white/5'}`}>
              <h2 className={`font-bold text-lg flex items-center gap-2 ${cText}`}><AlertTriangle className="h-5 w-5 text-orange-500" /> {t("File a Dispute")}</h2>
            ...
            <div className="p-5 flex-1 overflow-y-auto">
              ...
              {selectedReview.dispute ? (
                <div className={`p-4 rounded-xl border text-sm mb-4 ${theme === 'light' ? 'bg-amber-50 text-amber-800 border-amber-200' : 'bg-orange-500/10 text-orange-300 border-orange-500/20'}`}>
                  <p className="font-bold flex items-center gap-1"><AlertTriangle className="h-4 w-4"/> {t("Dispute Already Filed")}</p>
                  <p className="mt-2 font-medium">{t("Status:")} {t(selectedReview.dispute.status || "") || selectedReview.dispute.status}</p>
                  <p className="mt-1 font-medium">{t("Reason:")} {t(selectedReview.dispute.reason || "") || selectedReview.dispute.reason}</p>
                  <p className="mt-2 opacity-80">{t("You have already submitted a dispute for this review.")}</p>
                </div>
              ) : (
                <form id="dispute-form" onSubmit={handleDisputeSubmit} className="space-y-4">
                  <div>
                    <label className={`block text-sm font-bold mb-2 ${cText}`}>{t("Dispute Reason")}</label>
                    <select 
                      value={disputeReason} 
                    ...
                    >
                      <option value="Fake review">{t("Fake review")}</option>
                      <option value="Wrong Facebook page">{t("Wrong Facebook page")}</option>
                      <option value="Wrong bKash/contact number">{t("Wrong bKash/contact number")}</option>
                      <option value="False accusation">{t("False accusation")}</option>
                      <option value="Duplicate review">{t("Duplicate review")}</option>
                      <option value="Abusive content">{t("Abusive content")}</option>
                      <option value="Misleading screenshot/post link">{t("Misleading screenshot/post link")}</option>
                      <option value="Other">{t("Other")}</option>
                    </select>
                  </div>
                  <div>
                    <label className={`block text-sm font-bold mb-2 ${cText}`}>{t("Detailed Explanation")}</label>
                    <textarea 
                      rows={4}
                      value={disputeDescription}
                      onChange={e => setDisputeDescription(e.target.value)}
                      placeholder={t("Explain precisely why this review is incorrect and should be removed...")}
                      className={`w-full rounded-xl p-4 text-sm outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all ${cInput}`}
                      required
                    />
                  </div>
            ...
            <div className={`p-5 border-t flex justify-end gap-3 ${theme === 'light' ? 'bg-slate-50 border-slate-200' : 'bg-[#121A2F] border-white/5'}`}>
              <button disabled={submitting} type="button" onClick={() => setDisputeModalOpen(false)} className={`px-5 py-2.5 rounded-lg font-bold transition-colors ${theme === 'light' ? 'text-slate-600 hover:bg-slate-200' : 'text-slate-400 hover:bg-white/5'}`}>{selectedReview.dispute ? t('Close') : t('Cancel')}</button>
              {!selectedReview.dispute && (
                <button disabled={submitting} type="submit" form="dispute-form" className="px-5 py-2.5 rounded-lg font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50">
                  {submitting ? t('Submitting...') : t('Submit Dispute')}
                </button>
              )}
            </div>
          </div>

...
import { Store, Star, ShieldAlert, AlertTriangle, MessageSquare, Briefcase } from 'lucide-react';
...
export default function BusinessOverview() {
  ...
  if (loading) return <div className={`p-8 text-center font-medium ${cTextMuted}`}>Loading dashboard...</div>;
  ...
  if (!data || !data.claimedPages || data.claimedPages.length === 0) {
    ...
    return (
      <div className={`${cContainer} rounded-xl border p-8 text-center max-w-md mx-auto mt-10 space-y-5 shadow-sm`}>
        ...
        <div>
          <h2 className={`text-xl font-bold ${cText}`}>No Approved Pages</h2>
          <p className={`text-sm mt-1 leading-relaxed ${cTextMuted}`}>
            You don't have any active/approved business pages in your account yet.
          </p>
        </div>
        ...
        <div className={`p-3.5 rounded-xl border text-left ${theme === 'light' ? 'bg-amber-50 border-amber-200' : 'bg-amber-500/10 border-amber-500/20'}`}>
           <p className={`text-xs font-bold leading-normal ${theme === 'light' ? 'text-amber-800' : 'text-amber-400'}`}>
             💡 Just submitted a claim query?
           </p>
           <p className={`text-xs mt-0.5 leading-normal font-medium ${theme === 'light' ? 'text-amber-700' : 'text-amber-300'}`}>
             Your claim request is currently pending verification. You can check its review milestones on the <Link to="/business-dashboard/pages" className="underline font-bold">My Pages</Link> tab.
           </p>
        </div>
  ...
  const statCards = [
    { label: 'Fraud Reports', value: data.fraudReports, icon: AlertTriangle, color: theme === 'light' ? 'text-indigo-600' : 'text-indigo-400', bg: theme === 'light' ? 'bg-indigo-50' : 'bg-indigo-500/10' },
    { label: 'Total Reviews', value: data.totalReviews, icon: MessageSquare, color: theme === 'light' ? 'text-blue-600' : 'text-blue-400', bg: theme === 'light' ? 'bg-blue-50' : 'bg-blue-500/10' },
    { label: 'Average Rating', value: data.averageRating, icon: Star, color: theme === 'light' ? 'text-amber-500' : 'text-amber-400', bg: theme === 'light' ? 'bg-amber-50' : 'bg-amber-500/10' },
    { label: 'Pending Replies', value: data.pendingReplies, icon: MessageSquare, color: theme === 'light' ? 'text-orange-500' : 'text-orange-400', bg: theme === 'light' ? 'bg-orange-50' : 'bg-orange-500/10' },
    { label: 'Open Disputes', value: data.openDisputes, icon: AlertTriangle, color: theme === 'light' ? 'text-rose-500' : 'text-rose-400', bg: theme === 'light' ? 'bg-rose-50' : 'bg-rose-500/10' },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className={`text-2xl font-bold ${cText}`}>Overview</h1>
        <p className={cTextMuted}>Summary of your claimed Facebook business pages.</p>
      </div>
        ...
        {statCards.map((s, i) => (
          <div key={i} className={`${cContainer} p-5 rounded-xl border shadow-sm flex items-center gap-4`}>
            ...
            <div>
              ...
              <p className={`text-2xl font-black ${cText}`}>{s.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="space-y-6">
          <h2 className={`text-lg font-bold ${cText}`}>Recent Reviews</h2>
                     ...
                     <div className="flex justify-between items-start mb-2">
                        <div>
                           <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className={`text-xs ${cTextMuted}`}>{new Date(r.created_at).toLocaleDateString()}</span>
                           </div>
                        ...
                        <div className={`flex items-center px-2 py-1 rounded text-xs font-bold ${theme === 'light' ? 'bg-slate-100 text-slate-700' : 'bg-slate-800 text-slate-300'}`}>
                           <Star className="h-3 w-3 text-emerald-500 mr-1 fill-current" /> {r.star_rating}
                        </div>
                     </div>
                     <p className={`${theme === 'light' ? 'text-slate-600' : 'text-slate-400'} line-clamp-2 mt-1`}>{r.description}</p>
                     <div className="mt-3">
                        <Link to={`/business-dashboard/reviews?reviewId=${r.id}`} className="text-indigo-600 font-bold hover:text-indigo-400 hover:underline transition-colors">Reply \→</Link>
                     </div>
                   </div>
                ))}
              </div>
            ) : (
              <div className={`p-8 text-center ${cTextMuted}`}>No reviews yet.</div>
            )}
          </div>
        </div>
      </div>

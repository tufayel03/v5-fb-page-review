...
import { ExternalLink, Edit, Clock, AlertCircle, Facebook } from 'lucide-react';
...
export default function BusinessPages() {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 font-medium">{t("Loading pages...")}</div>;
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">{t("Claim Another Page")}</h1>
            <p className="text-slate-500">{t("Submit a request to manage another Facebook business page.")}</p>
          </div>
    ...
    <div className="space-y-10">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">{t("My Pages")}</h1>
            <p className="text-slate-500">{t("Manage your claimed and verified Facebook business pages.")}</p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {pages.length === 0 ? (
            <div className="col-span-2 p-10 bg-white text-center text-slate-500 rounded-xl border border-slate-200 shadow-sm">
              <h3 className="font-bold text-lg text-slate-800 mb-1">{t("No Claimed Pages Yet")}</h3>
              <p className="text-slate-400 text-sm mb-4">{t("You do not have any approved/active business page listings in your profile.")}</p>
              ...
              {pendingClaims.length > 0 && (
                <p className="text-indigo-600 text-sm font-semibold mt-4">{t("You have a claim request pending approval.")}</p>
              )}
            </div>
          ) : (
            pages.map(page => (
              <div key={page.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm border-t border-slate-100 pt-4">
                  <div>
                    <p className="text-slate-500">{t("Status Badge")}</p>
                    <p className="font-bold text-slate-900">{t(page.status_badge || "") || page.status_badge}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">{t("Reviews")}</p>
                    <p className="font-bold text-slate-900">{n(page.total_reviews)}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">{t("Average Rating")}</p>
                    <p className="font-bold text-slate-900">{n(page.average_rating)}</p>
                  </div>
                  <div>
                    <p className="text-slate-500">{t("Fraud Reports")}</p>
                    <p className="font-bold text-rose-600">{n(page.fraud_report_count)}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-100 pt-4 flex gap-2">
                  ...
                  <Link to="/business-dashboard/profile-info" className="flex-1 flex justify-center items-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-2 rounded-lg text-sm font-bold transition-colors">
                    <Edit className="h-4 w-4"/> {t("Edit Info")}
                  </Link>
                </div>
              </div>
            ))
        ...
        <div id="submitted-claims-section" className="space-y-4 border-t border-slate-200/80 pt-8">
          <div>
            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2 select-none">
              <Clock className="h-5 w-5 text-indigo-500" /> {t("Submitted Claim Requests")}
            </h2>
            <p className="text-sm text-slate-500">{t("These claims are pending manual administrator verification. You will gain profile access once approved.")}</p>
          </div>
          ...
          <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-4 md:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 dark:bg-amber-500/5">
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                <p className="text-sm font-extrabold text-amber-800 dark:text-amber-300">{t("Required Verification Action:")}</p>
                <p className="text-xs text-amber-700 dark:text-amber-200 leading-relaxed font-semibold">
                  {t("To verify ownership, you")} <strong className="font-extrabold text-amber-900 dark:text-amber-100">{t("must send a message")}</strong> {t("containing your")} <span className="underline font-black">{t("Claim Username")}</span> {t("or")} <span className="underline font-black">{t("Contact Email")}</span> {t("to our")}{" "}
                  {facebookPageUrl ? (
                    <a
                    ...
                    >
                      <Facebook className="h-3.5 w-3.5 fill-[#1877f2] dark:fill-[#5890ff] shrink-0" /> {t("Facebook Page")}
                    </a>
                  ) : (
                    <span className="font-extrabold">{t("Facebook Page")}</span>
                  )}{" "}
                  <strong className="font-extrabold text-amber-900 dark:text-amber-100">{t("directly from the Facebook Page you are claiming")}</strong>. {t("This allows us to confirm ownership and approve your access immediately.")}
                </p>
              </div>
            </div>
            {facebookPageUrl && (
              ...
              >
                {t("Send Message on Facebook")} <ExternalLink className="h-3 w-3" />
              </a>
            )}
          </div>
              ...
              <div key={claim.id} id={`claim-item-${claim.id}`} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-3 relative overflow-hidden">
                <div className="flex justify-between items-start gap-4">
                  <div className="min-w-0">
                    <h3 className="font-extrabold text-slate-800 text-md truncate">{claim.page_name || t('Unlisted Page')}</h3>
                    <a href={claim.facebook_url} target="_blank" rel="noreferrer" className="text-xs text-indigo-600 hover:underline flex items-center gap-1 mt-1 break-all">
                       {claim.facebook_url || t('Unknown URL')} <ExternalLink className="h-2.5 w-2.5 shrink-0 inline" />
                    </a>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-slate-600 bg-white border border-slate-100 p-3 rounded-xl shadow-xs">
                   <div>
                     <p className="text-slate-400 font-medium select-none">{t("Claim Username")}</p>
                     <p className="font-bold text-slate-700">{claim.claimer_username || '-'}</p>
                   </div>
                   <div>
                     <p className="text-slate-400 font-medium select-none">{t("Submitted At")}</p>
                     <p className="font-bold text-slate-700">{new Date(claim.created_at).toLocaleDateString(dateLocale)}</p>
                   </div>
                   {claim.contact_email && (
                     <div className="col-span-2 mt-1">
                       <p className="text-slate-400 font-medium select-none">{t("Contact Email")}</p>
                ...
                {claim.admin_note && (
                  <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-xl space-y-1 text-xs">
                     <p className="font-bold text-indigo-900 flex items-center gap-1 select-none">
                       <AlertCircle className="h-3.5 w-3.5 text-indigo-600" /> {t("Admin Note")}
                     </p>

...
<body>
  ...
  app.post('/api/admin/chrome-extension/sync-page-picture', requireModerator, async (req, res) => {
    try {
      ...
      const saveOptimizedPicture = async (buffer: Buffer) => {
        const timestamp = Date.now();
        const filename = `profile-${page.id}-${timestamp}.webp`;
        const filepath = path.join(uploadsDir, filename);
        await sharp(buffer).resize(300, 300, { fit: 'cover' }).webp({ quality: 80 }).toFile(filepath);
        const thumbFilename = `profile-thumb-${page.id}-${timestamp}.webp`;
        await sharp(buffer).resize(80, 80, { fit: 'cover' }).webp({ quality: 70 }).toFile(path.join(uploadsDir, thumbFilename));
        // Clean up old files
        ...
        db.prepare('UPDATE FacebookPages SET profile_picture = ?, current_name = COALESCE(?, current_name) WHERE id = ?').run(`/uploads/${filename}`, name || null, page.id);
        return `/uploads/${filename}`;
      };

      // PRIORITY: If extension sent the image bytes as base64, process directly — no VPS outbound request needed
      // This is the most reliable method since the browser fetches from the user's IP (not VPS datacenter IP)
      if (profilePictureBase64) {
        try {
          // Strip data URI prefix if present (e.g. "data:image/jpeg;base64,")
          const base64Data = profilePictureBase64.replace(/^data:[^;]+;base64,/, '');
          const buffer = Buffer.from(base64Data, 'base64');
          if (buffer.length < 200) throw new Error('Image data too small');
          const meta = await sharp(buffer).metadata();
          if (meta.width && meta.height && (meta.width < 50 || meta.height < 50)) throw new Error('Image too small (placeholder)');
          const savedPath = await saveOptimizedPicture(buffer);
          console.log(`[Sync Direct Base64] Successfully saved profile picture for page ${page.id}`);
          return res.json({ success: true, message: 'Profile picture successfully updated!', profilePictureUrl: savedPath });
        } catch (b64Err: any) {
          console.error('[Sync Direct Base64] Failed to process base64 image:', b64Err.message);
          // Fall through to URL-based method as fallback
        }
      }

      // FALLBACK: Try the URL (may work if CDN doesn't block this IP for this specific URL)
      if (profilePictureUrl && !profilePictureUrl.includes('silhouette') && !profilePictureUrl.includes('176159830277856') && !profilePictureUrl.includes('HsTZSDw4avx.gif') && !profilePictureUrl.includes('rsrc.php') && !profilePictureUrl.includes('static.xx.fbcdn.net')) {
        const tempFile = path.join(uploadsDir, `temp-ext-pic-${Date.now()}.jpg`);
        const downloaded = await downloadUrlToTempFile(profilePictureUrl, tempFile);
        if (downloaded) {
          try {
            ...
            console.error('[Sync Direct] Profile picture optimization failed:', optErr.message);
            try { fs.unlinkSync(tempFile); } catch (e) {}
          }
        }
      }
    ...
    try {
      ...
      if (!hasPagePluginSuccess) {
        ...
        try {
          ...
          // Try 1: Native Node fetch with default headers (highly successful for profile bypasses)
          try {
            console.log(`[AutoScrape] Direct fetch attempt 1 (Native Node fetch, default headers): ${urlNoSlash}`);
            const res = await fetch(addFacebookLocale(urlNoSlash), {
              headers: {
                'Accept-Language': 'en-US,en;q=0.9',
              },
              signal: AbortSignal.timeout(8000)
            ...
            if (res.ok) {
              ...
              const titleMatch = tempHtml.match(/<title[^>]*>([^<]+)<\/title>/i);
              const ogTitleMatch = tempHtml.match(/<meta[^>]*(?:property|name)=["']og:title["'][^>]*content=["']([^"']+)["']/i);
              const parsedTitle = (ogTitleMatch ? ogTitleMatch[1] : (titleMatch ? titleMatch[1] : '')).toLowerCase().trim();
              const isRoadblocked = !parsedTitle || 
                                    isFallbackFacebookName(parsedTitle) || 
                                    tempHtml.includes("This content isn't available") || 
                                    tempHtml.includes("isn't available at the moment");

              if (!isRoadblocked) {
                html = tempHtml;
                isDirectSuccess = true;
                console.log(`[AutoScrape] Direct fetch attempt 1 succeeded. Resolved title: "${parsedTitle}"`);
              }
            }
          } catch (e: any) {
            console.error('[AutoScrape] Direct fetch attempt 1 failed:', e.message);
          }

          // Try 2: Native Node fetch with configured cookies (if Attempt 1 was blocked/failed)
          if (!isDirectSuccess && scraperCookie) {
            try {
              console.log(`[AutoScrape] Direct fetch attempt 2 (Native Node fetch, cookies): ${urlNoSlash}`);
              const res = await fetch(addFacebookLocale(urlNoSlash), {
                headers: {
                  'Cookie': scraperCookie,
                  'Accept-Language': 'en-US,en;q=0.9',
                },
                signal: AbortSignal.timeout(8000)
              });
              if (res.ok) {
                ...
                if (!isRoadblocked) {
                  html = tempHtml;
                  isDirectSuccess = true;
                  console.log(`[AutoScrape] Direct fetch attempt 2 succeeded. Resolved title: "${parsedTitle}"`);
                }
              }
            } catch (e: any) {
              console.error('[AutoScrape] Direct fetch attempt 2 failed:', e.message);
            }
          }

          // Fallback: Google Translate Proxy (via native Node fetch)
          if (!isDirectSuccess) {
            console.log(`[AutoScrape] Direct fetch failed or was blocked. Bypassing via Google Translate proxy...`);
            try {
              const proxyUrl = `https://translate.google.com/translate?sl=auto&tl=en&u=${encodeURIComponent(addFacebookLocale(urlNoSlash))}`;
          ...
          if (html) {
            ...
            if (ogTitle) {
              ...
              const isRoadblocked = !cleanedTitle ||
                isFallbackFacebookName(cleanedTitle) ||
                html.includes("This content isn't available") ||
                html.includes("isn't available at the moment");
      ...
      if (!profilePicture && username) {
        ...
        try {
          ...
          const imgRes = await fetch(graphPicUrl, { signal: AbortSignal.timeout(8000) });
          if (imgRes.ok) {
            const arrayBuffer = await imgRes.arrayBuffer();
            const imageBuffer = Buffer.from(arrayBuffer);
